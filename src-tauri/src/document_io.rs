...
pub(crate) struct ConditionalWriteResult {
    ...
    pub(crate) current_revision: FileRevision,
    canonical_path: String,
    name: String,
...
pub(crate) enum NewMarkdownFile {
    Written(ConditionalWriteResult),
    AlreadyExists,
}

pub(crate) fn reject_oversized_markdown(content: &str) -> Result<(), NativeFileError> {
...
fn occupied_markdown_name_error(path: &Path) -> NativeFileError {
    let file_name = path
        .file_name()
    ...
    NativeFileError::invalid(
        ...
        format!("{file_name} already exists. Choose a different name."),
    )
}

/// Creates a new document without replacing an occupied name. macOS/Linux use
/// the descriptor-bound writer; other platforms retain the legacy reservation path.
pub(crate) fn write_new_markdown_file(
    path: &Path,
    content: &str,
) -> Result<NewMarkdownFile, NativeFileError> {
    reject_oversized_markdown(content)?;
    // Any existing directory entry occupies the name, including a symlink or directory.
    // Do not follow it. A later exclusive create still covers the race after this check.
    match fs::symlink_metadata(path) {
        Ok(_) => return Ok(NewMarkdownFile::AlreadyExists),
    ...
    {
        // Resolve the chosen parent, but never follow a final symlink that may
        // have appeared after the occupied-name check above.
        let write_path = resolve_markdown_write_name(path)?;
        crate::document_write::create_document(&write_path, content)
    }
    #[cfg(not(any(target_os = "macos", target_os = "linux")))]
    {
        let write_path = resolve_markdown_write_target(path)?;
            ...
            .open(&write_path)
        {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
                return Ok(NewMarkdownFile::AlreadyExists);
            }
            Err(error) => {
                return Err(NativeFileError::from_io(
                    NativeFileOperation::SaveDocument,
        ...
        Ok(NewMarkdownFile::Written(write_reserved_markdown(
            &write_path,
            content,
        )?))
    }
...
#[cfg(not(any(target_os = "macos", target_os = "linux")))]
pub(crate) fn write_reserved_markdown(
    path: &Path,
    content: &str,
) -> Result<ConditionalWriteResult, NativeFileError> {
    if let Err(error) = write_markdown_contents_atomic(path, content) {
        let _ = fs::remove_file(path);
        return Err(error);
    }
    successful_write_result(path, content)
}
...
pub(crate) async fn write_markdown_file_if_unmodified(
    ...
    run_blocking_file_io(move || {
        ...
        if create_new.unwrap_or(false) {
            return match write_new_markdown_file(&requested_path, &content)? {
                NewMarkdownFile::Written(result) => Ok(result),
                NewMarkdownFile::AlreadyExists => {
                    Err(occupied_markdown_name_error(&requested_path))
                }
            };
        }
...
fn write_markdown_file_if_unmodified_impl(
    ...
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    {
        // A current document is already canonical. Only an explicit Save As
        // (no expected revision) may select a new symlink destination.
        let write_path = if force_save && expected_revision.is_none() {
            resolve_markdown_write_target(&requested_path)?
        } else {
            requested_path
        };
        crate::document_write::write_document(
            &write_path,
            &content,
...
pub(crate) fn conditional_write_result(
    path: &Path,
    conflict: bool,
...
pub(crate) fn stable_hash_hex(bytes: &[u8]) -> String {
    ...
    // Callers pair these hashes with source identity or file revision metadata.
...
#[cfg(not(any(target_os = "macos", target_os = "linux")))]
pub(crate) fn successful_write_result(
    path: &Path,
    content: &str,
) -> Result<ConditionalWriteResult, NativeFileError> {
...
#[cfg(any(test, not(any(target_os = "macos", target_os = "linux"))))]
pub(crate) fn write_markdown_contents_atomic(
    path: &Path,
    content: &str,
) -> Result<(), NativeFileError> {
...
mod tests {
    ...
    fn write_rejects_symlink_to_non_markdown() {
        ...
        let error = result.expect_err("an unexpected symlink must not be followed");
        assert_eq!(error.category, NativeFileErrorCategory::InvalidInput);
        assert_eq!(fs::read_to_string(&target).unwrap(), "plain text");
    ...
    #[test]
    fn create_new_save_refuses_an_existing_file_and_writes_a_free_name() {
        let root = temp_dir("create-new-save");
        fs::create_dir(&root).expect("create fixture root");
        let existing = root.join("Chapter.md");
        fs::write(&existing, "keep").expect("write existing file");

        assert!(matches!(
            write_new_markdown_file(&existing, "replace").expect("classify the existing name"),
            NewMarkdownFile::AlreadyExists
        ));
        let error = occupied_markdown_name_error(&existing);
        assert!(error.message.contains("Chapter.md already exists"));
        assert_eq!(
            fs::read_to_string(&existing).expect("read existing file"),
            "keep"
        );

        let fresh = root.join("Notes.md");
        let NewMarkdownFile::Written(result) =
            write_new_markdown_file(&fresh, "notes").expect("write a free name")
        else {
            panic!("a free name was reported as occupied");
        };
        assert!(!result.conflict);
        assert_eq!(fs::read_to_string(&fresh).expect("read new file"), "notes");

        cleanup_dir(&root);
    }

    fn temp_path(ext: &str) -> PathBuf {
        canonical_temp_parent(unique_temp_path(ext))
    }

    fn temp_dir(prefix: &str) -> PathBuf {
        canonical_temp_parent(unique_temp_dir(prefix))
    }

    fn canonical_temp_parent(path: PathBuf) -> PathBuf {

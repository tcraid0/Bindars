...
impl NativeFileError {
    /// Creation succeeded, but writing, syncing or inspecting that file failed.
    /// Preserve this distinction across IPC: an OS error category alone cannot
    /// tell the editor that the destination may already contain partial text.
    pub(crate) fn incomplete_write(path: &Path, error: std::io::Error) -> Self {
        let cause = Self::from_io(NativeFileOperation::SaveDocument, path, error);
        Self {
    ...
    /// The pathname no longer names the directory this save was using.
    /// `detail` is the stable marker the editor uses to refuse another save there.
    pub(crate) fn destination_changed(message: impl Into<String>) -> Self {
        Self {
            category: NativeFileErrorCategory::InvalidInput,
            operation: NativeFileOperation::InspectWriteTarget,
            message: message.into(),
            detail: "destination-changed".to_string(),
        }
    }
    ...
    pub(crate) fn from_io(
        ...
        let category = match error.kind() {
            ...
            std::io::ErrorKind::AlreadyExists => NativeFileErrorCategory::AlreadyExists,

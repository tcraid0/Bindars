    ...
    match result {
        Ok(event) => {
            ...
            if matches!(
                ...
                .paths
                .iter()
                .any(|path| watched_path.parent() == Some(dunce::simplified(path)))
            {
                return Some(WatcherNotification::Unavailable(
                    "the watched document's folder moved or became unavailable".to_string(),
                ));
            }
...
mod tests {
    ...
    #[test]
    fn parent_moves_and_removals_report_watch_health_loss() {
        use notify::{
            event::{ModifyKind, RemoveKind, RenameMode},
            Event, EventKind,
        };
        let path = Path::new("/tmp/folder/document.md");
        for kind in [
            EventKind::Modify(ModifyKind::Name(RenameMode::From)),
            EventKind::Remove(RemoveKind::Folder),
        ] {
            let event = Event::new(kind).add_path(path.parent().unwrap().to_path_buf());
            assert!(matches!(
                classify_watcher_result(Ok(event), path),
                Some(WatcherNotification::Unavailable(_))
            ));
            let sibling = Event::new(kind).add_path(PathBuf::from("/tmp/folder/other.md"));
            assert_eq!(classify_watcher_result(Ok(sibling), path), None);
        }
    }

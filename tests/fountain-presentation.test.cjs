...
test("exhausted emphasis lookahead stays literal on that line and resumes on the next", () => {
  const prefix = ("_" + "a".repeat(100) + " ").repeat(40);
  assert.deepEqual(segments(prefix + "*literal*\n*italic* and _underlined_"), [
    prefix + "*literal*\n", "italic(italic)", " and ", "underline(underlined)",
  ]);
});
...
test("character extensions are stripped only from the end of the cue", () => {
  ...
  assert.equal(normalizeCharacterName("BOB (A) X (B)"), "BOB (A) X");
  assert.equal(normalizeCharacterName("BOB ((V.O.)  "), "BOB");
  assert.equal(normalizeCharacterName("BOB (V.O.)*"), "BOB (V.O.)*");
});

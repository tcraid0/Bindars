...
test("watcher health stays degraded until a replacement watch succeeds", async () => {
  const rendered = await renderFileWatcher({ failWatch: true });
  try {
    await waitFor(() => assert.equal(rendered.api().unavailable, true));
    await act(async () => rendered.api().retry());
    await waitFor(() => assert.equal(rendered.unavailable.length, 2));
    assert.equal(rendered.api().unavailable, true);
    rendered.allowWatch();
    await act(async () => rendered.api().retry());
    await waitFor(() => assert.equal(rendered.api().unavailable, false));
    await act(async () => emit(FILE_WATCHER_UNAVAILABLE_EVENT, { path: "/tmp/unrelated.md" }));
    assert.equal(rendered.api().unavailable, false);
    await act(async () => emit(FILE_WATCHER_UNAVAILABLE_EVENT, { path: FILE_PATH }));
    assert.equal(rendered.api().unavailable, true);

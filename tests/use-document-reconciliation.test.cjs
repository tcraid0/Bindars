...
for (const startedBeforeSave of [false, true]) {
  test(`reconciliation waits for a save revision (probe already started: ${startedBeforeSave})`, async () => {
    await installDom();
    const original = snapshot({ mode: "editor", dirty: true, content: "Local", expectedRevision: revision });
    const rendered = renderReconciliationHook(original);
    try {
      let pending;
      if (startedBeforeSave) pending = rendered.api().requestReconciliation("focus");
      rendered.setState({ ...original, saveInFlight: true });
      if (startedBeforeSave) {
        await resolveProbe(rendered.probes[0], available("Local", { ...revision, contentHash: "saved" }));
        assert.equal((await pending).kind, "stale-noop");
      } else {
        assert.deepEqual(await rendered.api().requestReconciliation("focus"), { kind: "deferred" });
      }
      assert.equal(rendered.applied.length, 0);
      const count = rendered.probes.length;
      rendered.api().resumeDeferredReconciliation();
      assert.equal(rendered.probes.length, count);
      const saved = { mtimeMs: 2, size: 5, contentHash: "saved" };
      rendered.setState({ ...original, content: "Newer typing", saveInFlight: false, expectedRevision: saved, publishedRevision: saved });
      rendered.api().resumeDeferredReconciliation();
      assert.equal(rendered.probes.length, count + 1);
      await resolveProbe(rendered.probes.at(-1), available("Local", saved));
      assert.equal(rendered.applied.at(-1).decision.kind, "no-change");
    } finally { rendered.cleanup(); }
  });
}

test("a read started before saving is retried if it returns after the saved revision was published", async () => {
  await installDom();
  const original = snapshot({ mode: "editor", dirty: true, content: "Local", expectedRevision: revision });
  const rendered = renderReconciliationHook(original);
  try {
    const pending = rendered.api().requestReconciliation("focus");
    const saved = { mtimeMs: 2, size: 5, contentHash: "saved" };
    rendered.setState({ ...original, content: "Newer typing", saveInFlight: false, expectedRevision: saved, publishedRevision: saved });
    await resolveProbe(rendered.probes[0], available("Original", revision));
    assert.equal((await pending).kind, "stale-noop");
    assert.equal(rendered.applied.length, 0);
    assert.equal(rendered.probes.length, 2);
    await resolveProbe(rendered.probes[1], available("Local", saved));
    assert.equal(rendered.applied.at(-1).decision.kind, "no-change");
  } finally { rendered.cleanup(); }
});

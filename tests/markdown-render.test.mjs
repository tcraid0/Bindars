...
test("MarkdownRenderer recognises mermaid fences regardless of case", () => {
  const html = renderMarkdownRenderer("```Mermaid\ngraph TD; A-->B;\n```");
...
test("frontmatter after a leading byte-order mark is still recognised", () => {
  const plain = "---\ntitle: Field notes\nauthor: Ada\n---\n\n# Body heading\n\nText.\n";
  const withBom = `\uFEFF${plain}`;

  const expected = extractFrontmatter(plain);
  const actual = extractFrontmatter(withBom);
  assert.equal(actual.frontmatter?.title, "Field notes");
  assert.equal(actual.body, expected.body);
  assert.equal(actual.bodyStartLine, expected.bodyStartLine);

  const html = renderMarkdownRenderer(withBom);
  assert.match(html, /Field notes/);
  assert.match(html, /Ada/);
  assert.doesNotMatch(html, /title: Field notes/);

  assert.equal(buildWorkspaceDoc(buildMeta("bom.md"), withBom).title, "Field notes");
});

...
function startSaveAs(rendered, defaultPath = "Untitled.md", currentPath = null) {
  let savePromise;
  flushSync(() => {
    savePromise = rendered.api().saveAs(defaultPath, currentPath);
  });
  return savePromise;
}

function startCreateDraft(rendered) {
  let savePromise;
  flushSync(() => {
    savePromise = rendered.api().createDraft();
  });
  return savePromise;
}
...
function mockPendingWrites({ allowDrafts = false } = {}) {
  const writes = [];
  mockIPC((cmd, args) => {
    if (cmd !== "write_markdown_file_if_unmodified"
      && !(allowDrafts && cmd === "create_draft_document")) {
...
test("createDraft saves without a dialog and uses its revision for later conditional saves", async () => {
  await installDom();
  const writes = mockPendingWrites({ allowDrafts: true });
  const rendered = renderUseEditor();

  try {
    const draftPath = "/tmp/Bindars Drafts/Untitled.md";
    const draftRevision = { mtimeMs: 2, size: 11, contentHash: "draft" };
    assert.deepEqual(await rendered.api().createDraft(), { status: "noop" });
    enterEditMode(rendered, "", null);
    updateBuffer(rendered, "Draft words");
    const draftSave = startCreateDraft(rendered);

    assert.equal(rendered.api().saving, true);
    assert.equal(writes.length, 1);
    assert.equal(writes[0].cmd, "create_draft_document");
    assert.deepEqual(writes[0].args, { content: "Draft words" });
    assert.deepEqual(await rendered.api().createDraft(), { status: "noop" });
    assert.deepEqual(await rendered.api().saveAs("Duplicate.md"), { status: "noop" });
    assert.deepEqual(await rendered.api().save(draftPath), { status: "noop" });

    const result = await settleSave(writes[0], draftSave, successfulWrite(draftRevision, draftPath));
    assert.equal(result.status, "saved");
    assert.equal(result.file.canonicalPath, draftPath);
    assert.equal(result.file.content, "Draft words");
    assert.equal(rendered.api().dirty, false);
    assert.equal(rendered.api().saving, false);

    updateBuffer(rendered, "Draft words again");
    const nextSave = startSave(rendered, draftPath);
    assert.equal(writes[1].cmd, "write_markdown_file_if_unmodified");
    assert.deepEqual(writes[1].args.expectedRevision, draftRevision);
    assert.equal(writes[1].args.force, false);
    assert.equal((await settleSave(
      writes[1],
      nextSave,
      successfulWrite({ mtimeMs: 3, size: 17, contentHash: "updated" }, draftPath),
    )).status, "saved");
  } finally {
    rendered.cleanup();
  }
...
test("createDraft captures pending surface text and preserves typing during the write", async () => {
  await installDom();
  const writes = mockPendingWrites({ allowDrafts: true });
  const rendered = renderUseEditorWithSurface("");

  try {
    enterEditMode(rendered, "", null);
    rendered.setLiveContent("Pending before draft");
    const draftSave = startCreateDraft(rendered);
    assert.equal(rendered.flushCount(), 1);
    assert.equal(writes[0].args.content, "Pending before draft");
    assert.deepEqual(await rendered.api().createDraft(), { status: "noop" });
    assert.equal(rendered.flushCount(), 1, "a duplicate draft save must not flush the surface");

    rendered.setLiveContent("Pending while draft writes");
    const result = await settleSave(
      writes[0],
      draftSave,
      successfulWrite({ mtimeMs: 2, size: 20, contentHash: "draft" }),
    );
    assert.equal(result.status, "saved-with-newer-edits");
    assert.equal(result.file.content, "Pending before draft");
    assert.equal(rendered.flushCount(), 2);
    assert.equal(rendered.api().buffer, "Pending while draft writes");
    assert.equal(rendered.api().dirty, true);
  } finally {
    rendered.cleanup();
  }
...
test("createDraft failure stays quiet, preserves newer text, and releases the save guard", async () => {
  await installDom();
  const writes = mockPendingWrites({ allowDrafts: true });
  const rendered = renderUseEditorWithSurface("");
  ...
  try {
    ...
    updateBuffer(rendered, "Draft words");
    const draftSave = startCreateDraft(rendered);
    rendered.setLiveContent("Newer words after draft starts");

    const result = await rejectSave(writes[0], draftSave, new Error("Documents access denied"));
    assert.deepEqual(result, { status: "error" });
    assert.equal(rendered.api().saveError, null);
    assert.equal(rendered.api().saveErrorRecovery, null);
    assert.equal(rendered.api().buffer, "Newer words after draft starts");
    assert.equal(rendered.api().dirty, true);
    assert.equal(rendered.api().saving, false);

    const retry = startCreateDraft(rendered);
    assert.equal(writes.length, 2);
    assert.equal(writes[1].args.content, "Newer words after draft starts");
    assert.equal((await settleSave(
      writes[1],
      retry,
      successfulWrite({ mtimeMs: 2, size: 29, contentHash: "retry" }),
    )).status, "saved");
  } finally {
    rendered.cleanup();
  }
...
for (const completion of ["success", "failure"]) {
  test(`a stale createDraft ${completion} cannot change or release a newer session save`, async () => {
    await installDom();
    const writes = mockPendingWrites({ allowDrafts: true });
    ...
    try {
      ...
      const oldSave = startCreateDraft(rendered);
      enterEditMode(rendered, "Second", null);
      const newSave = startCreateDraft(rendered);
      assert.equal(writes.length, 2, "the new session can save while the old write finishes");
      rendered.setLiveContent("Second unpublished words");
      const oldResult = completion === "success"
        ? await settleSave(writes[0], oldSave, successfulWrite({ mtimeMs: 2, size: 5, contentHash: "old" }))
        : await rejectSave(writes[0], oldSave, new Error("Old draft failed"));

      assert.deepEqual(oldResult, { status: "stale" });
      assert.equal(rendered.flushCount(), 2, "the old completion must not flush the new session");
      assert.equal(rendered.api().buffer, "Second");
      assert.equal(rendered.api().saving, true);
      assert.equal(rendered.api().saveError, null);
      assert.deepEqual(await rendered.api().createDraft(), { status: "noop" });
      assert.equal(writes.length, 2, "the new session still owns its duplicate-save guard");

      const result = await settleSave(
        writes[1],
        newSave,
        successfulWrite({ mtimeMs: 3, size: 6, contentHash: "new" }, "/tmp/Untitled 2.md"),
      );
      assert.equal(result.status, "saved-with-newer-edits");
      assert.equal(rendered.api().buffer, "Second unpublished words");
      assert.equal(rendered.api().saving, false);
    } finally {
      rendered.cleanup();
    }
...
test("a retained competing version stays visible through newer typing and pauses even quiet saves", async () => {
  await installDom();
  const writes = mockPendingWrites();
  const rendered = renderUseEditor();
  try {
    enterEditMode(rendered, "Draft");
    updateBuffer(rendered, "Local words");
    const save = startSave(rendered, "/tmp/draft.md", { quiet: true });
    assert.equal(rendered.api().getReconciliationState().saveInFlight, true);
    updateBuffer(rendered, "Even newer local words");
    const writtenRevision = { mtimeMs: 2, size: 11, contentHash: "local" };
    const recoveryPath = "/tmp/Bindars recovered race.md";
    const result = await settleSave(writes[0], save, {
      ...successfulWrite(writtenRevision, "/tmp/draft.md"),
      recoveryPath,
    });
    assert.equal(result.status, "saved-with-recovery");
    assert.equal(result.recoveryPath, recoveryPath);
    assert.equal(result.file.canonicalPath, "/tmp/draft.md");
    assert.deepEqual(result.file.revision, writtenRevision);
    assert.equal(result.file.content, "Local words");
    assert.equal(rendered.api().getReconciliationState().saveInFlight, false);
    assert.deepEqual(rendered.api().getReconciliationState().expectedRevision, writtenRevision);
    assert.equal(rendered.api().buffer, "Even newer local words");
    assert.equal(rendered.api().dirty, true);
    assert.equal(rendered.api().recoveryPath, recoveryPath);
    flushSync(() => rendered.api().dismissSaveError());
    updateBuffer(rendered, "Still newer local words");
    assert.equal(rendered.api().recoveryPath, recoveryPath);

    const nextSave = startSave(rendered, "/tmp/draft.md");
    assert.equal(writes[1].args.content, "Still newer local words");
    assert.deepEqual(writes[1].args.expectedRevision, writtenRevision);
    assert.equal(writes[1].args.path, "/tmp/draft.md");
    const saved = await settleSave(
      writes[1],
      nextSave,
      successfulWrite({ mtimeMs: 3, size: 24, contentHash: "newer" }, "/tmp/draft.md"),
    );
    assert.equal(saved.status, "saved");
    assert.equal(rendered.api().recoveryPath, null);
    assert.deepEqual(
      rendered.api().getReconciliationState().expectedRevision,
      { mtimeMs: 3, size: 24, contentHash: "newer" },
    );
  } finally { rendered.cleanup(); }
});

test("a moved folder cannot be saved again at the same pathname", async () => {
  await installDom();
  const writes = mockPendingWrites();
  const rendered = renderUseEditor();
  try {
    ...
    assert.equal((await rejectSave(writes[0], save, moved)).status, "error");
    assert.equal(rendered.api().savePathBlocked, true);
    assert.match(rendered.api().saveError, /original-folder/);
    updateBuffer(rendered, "Local words plus more");
    flushSync(() => rendered.api().dismissSaveError());
    assert.match(rendered.api().saveError, /original-folder/);
    assert.equal(rendered.api().dirty, true);
    const refused = startSave(rendered, "/tmp/draft.md");
    assert.equal(writes.length, 1);
    assert.equal((await refused).status, "error");
    assert.match(rendered.api().saveError, /original-folder/);
  } finally { rendered.cleanup(); }
});

for (const selectedPath of ["/tmp/draft.md", "/tmp/another.md"]) {
  test(`Save As with recovery clears a moved-folder block when adopting ${selectedPath}`, async () => {
    await installDom();
    ...
    try {
      ...
      updateBuffer(rendered, "Local words");
      const firstSave = startSave(rendered);
      await rejectSave(writes[0], firstSave, {
        category: "invalidInput", operation: "inspectWriteTarget",
        message: "The folder moved during saving.", detail: "destination-changed",
      });
      const refused = startSave(rendered, "/tmp/draft.md", { force: true });
      assert.equal((await refused).status, "error");
      assert.equal(writes.length, 1);

      const saveAs = startSaveAs(rendered);
      await resolveDialog(dialogs[0], selectedPath);
      const revision = { mtimeMs: 42, size: 11, contentHash: "local" };
      const recoveryPath = "/tmp/Bindars recovered other.md";
      const result = await settleSave(writes[1], saveAs, {
        ...successfulWrite(revision, selectedPath), recoveryPath,
      });
      assert.equal(result.status, "saved-with-recovery");
      assert.equal(result.file.canonicalPath, selectedPath);
      assert.equal(rendered.api().savePathBlocked, false);
      assert.equal(rendered.api().recoveryPath, recoveryPath);
      assert.equal(rendered.api().dirty, true);

      updateBuffer(rendered, "Local words plus newer typing");
      const nextSave = startSave(rendered, selectedPath);
      assert.equal(writes.length, 3);
      assert.deepEqual(writes[2].args.expectedRevision, revision);
      assert.equal(writes[2].args.path, selectedPath);
      assert.equal(writes[2].args.content, "Local words plus newer typing");
      await settleSave(writes[2], nextSave, successfulWrite(
        { mtimeMs: 43, size: 28, contentHash: "newer" }, selectedPath,
      ));
      assert.equal(rendered.api().recoveryPath, null);
    } finally { rendered.cleanup(); }
  });
...
test("metadata retry never adopts a different document path", async () => {
  await installDom();
  const writes = mockPendingWrites();
  const rendered = renderUseEditor();
  try {
    enterEditMode(rendered, "Draft");
    updateBuffer(rendered, "Local words");
    const save = startSave(rendered);
    const result = await settleSave(writes[0], save,
      conflictingWrite({ ...originalRevision, mtimeMs: 2 }, "/tmp/another.md"));
    assert.equal(result.status, "conflict");
    assert.equal(writes.length, 1);
    assert.equal(rendered.api().buffer, "Local words");
  } finally { rendered.cleanup(); }
...
for (const operation of ["resolveDocument", "resolveWriteParent"]) {
  test(`missing path at ${operation} offers Save As even for autosave`, async () => {
    await installDom();
    const writes = mockPendingWrites();
    const rendered = renderUseEditor();
    try {
      ...
      updateBuffer(rendered, "Local words");
      const save = startSave(rendered, "/tmp/draft.md", { quiet: true });
      assert.deepEqual(await rejectSave(writes[0], save, {
        category: "notFound", operation, message: "Missing path", detail: "ENOENT",
      }), { status: "error" });
      assert.equal(rendered.api().saveErrorRecovery, "save-as");
      assert.match(rendered.api().saveError, /no longer available/);
      assert.doesNotMatch(rendered.api().saveError, /deleted|[Oo]verwrite/);
      assert.equal(rendered.api().buffer, "Local words");
      assert.equal(rendered.api().dirty, true);
    } finally { rendered.cleanup(); }
  });

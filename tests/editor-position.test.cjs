...
test("fragment lookup prefers literal IDs before decoding a heading link once", async () => {
  await installDom();
  const root = document.createElement("article");
  root.innerHTML = '<p id="user-content-fn-%61">Percent</p><p id="user-content-fn-a">A</p>'
    + '<p id="user-content-fn-caf%C3%A9">Note</p><h2 id="café">Heading</h2>'
    + '<p id="bad%ZZ">Malformed</p>';
  for (const [fragment, text] of [
    ["user-content-fn-%61", "Percent"], ["user-content-fn-a", "A"],
    ["user-content-fn-caf%C3%A9", "Note"], ["user-content-fn-caf%c3%a9", "Note"],
    ["user-content-fn-café", "Note"], ["caf%C3%A9", "Heading"],
    ["café", "Heading"], ["bad%ZZ", "Malformed"],
  ]) assert.equal(findFragmentElement(root, fragment)?.textContent, text);
  assert.equal(findFragmentElement(root, "caf%25C3%25A9"), null, "do not decode twice");
});

...
test("a path retargeted to another document is visibly unavailable and never adopted", async () => {
  const read = deferred();
  read.resolve({ content: "Same contents", canonicalPath: "/tmp/other.md", name: "other.md",
    revision: { mtimeMs: 1, size: 13, contentHash: "same" } });
  const result = await probeDocumentForReconciliation("/tmp/original.md", () => startedRead(read));
  assert.equal(result.status, "unavailable");
  assert.match(result.error.message, /different location.*Save As/);
});

...
test('record state reports empty only for a loaded, saved record without items', async (t) => {
  const { disk } = backingStore(t, new Map([['/a.md', record()], ['/unreadable.md', { highlights: [42], bookmarks: [] }]]));
  const read = deferred();
  t.mock.method(storage, 'loadAnnotations', () => read.promise);
  const view = await mount(t, '/empty.md');
  assert.equal(view.api().recordState('/never-opened.md'), 'unknown');
  assert.equal(view.api().recordState('/empty.md'), 'unknown', 'still loading');
  await act(async () => read.reject(new Error('unavailable')));
  assert.equal(view.api().recordState('/empty.md'), 'unknown', 'failed load');
  t.mock.method(storage, 'loadAnnotations', async (path) => copy(disk.get(path) ?? null));
  await view.change(api => api.retryLoad());
  assert.equal(view.api().recordState('/empty.md'), 'empty');
  await view.render('/unreadable.md');
  assert.equal(view.api().recordState('/unreadable.md'), 'annotated', 'retained unreadable items count');
  await view.render('/a.md');
  assert.equal(view.api().recordState('/a.md'), 'annotated');
  t.mock.method(storage, 'saveAnnotations', async () => { throw new Error('disk full'); });
  await view.change(api => api.removeHighlight('h'));
  assert.equal(view.api().recordState('/a.md'), 'unknown', 'an unsaved removal leaves the old notes on disk');
  t.mock.method(storage, 'saveAnnotations', async (path, data) => { disk.set(path, copy(data)); });
  await view.change(api => api.retrySave());
  assert.equal(view.api().recordState('/a.md'), 'empty');
});

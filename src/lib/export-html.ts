import { clearMarks, isAnnotationMark, isSearchMark } from "./dom-marks";
...
export function serializeExportRoot(root: Element): string {
  const clone = root.cloneNode(true) as HTMLElement;
  clearMarks(clone, (element) => isSearchMark(element) || isAnnotationMark(element));
  for (const element of [clone, ...clone.querySelectorAll<HTMLElement>("*")]) {
    for (const attribute of Array.from(element.attributes)) {
      if (attribute.name.startsWith("data-bindars-source-")) {

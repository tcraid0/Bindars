import { Lexer, rules } from "fountain-js";
...
import { sourcePointAtOffset } from "./source-lines";
...
export function parseFountain(
  ...
  // The reader renders tokens itself; Fountain.parse also builds unused HTML.
  const [titlePageTokens, scriptTokens] = Lexer.tokenize(text);
  const parsedTokens = [...titlePageTokens, ...scriptTokens];

  const titlePage: FountainTitlePageEntry[] = [];
  const tokens: FountainToken[] = [];
  const scenes: FountainScene[] = [];
  const sceneSourceCandidates = findFountainSceneSourceCandidates(text, parsedTokens);
...
function fountainSourceBlocks(content: string): FountainSourceBlock[] {
  const normalized = content
    // The library's stripper decides which comments are closed; blank those in
    // place so offsets survive, and keep unclosed ones as literal text.
    .replace(rules.boneyard, (match) => Lexer.boneyardStripper(match) === match ? match : match.replace(/[^\r\n]/g, " "))
    .replace(/\r\n|\r/g, "\n");
...
function findFountainSceneSourceCandidates(
  ...
  for (const block of fountainSourceBlocks(content)) {
    ...
    // The heading may follow whitespace-only lines inside its block.
    const withinBlock = sourcePointAtOffset(block.text, Math.max(0, block.text.indexOf(captured)));
    candidates.push({
      text: normalized,
      source: withinBlock.line === 1
        ? { line: block.source.line, column: block.source.column + withinBlock.column - 1 }
        : { line: block.source.line + withinBlock.line - 1, column: withinBlock.column },
    });
  }
...
/**
 * Extensions such as (V.O.), (CONT'D) or (INTO PHONE) trail the cue. Strip
 * them by index: an unanchored regex rescans every group from each start.
 */
export function normalizeCharacterName(raw: string): string {
  let end = endBeforeWhitespace(raw, raw.length);
  while (raw[end - 1] === ")") {
    const open = raw.indexOf("(", raw.lastIndexOf(")", end - 2) + 1);
    if (open < 0 || open >= end - 1) break;
    end = endBeforeWhitespace(raw, open);
  }
  return raw.slice(0, end).trim().toUpperCase();
}

function endBeforeWhitespace(text: string, end: number): number {
...
export function splitFountainInline(text: string): FountainInlineSegment[] {
  return splitInline(text, PLAIN_INLINE_STYLE, { remaining: lineLookaheadBudget(text, 0) });
}

/** Bound repeated lookahead per line; exhausted markers stay literal. */
function lineLookaheadBudget(text: string, lineStart: number): number {
  const newline = text.indexOf("\n", lineStart);
  return ((newline === -1 ? text.length : newline) - lineStart) * 4 + 1024;
}
...
function splitInline(text: string, style: InlineStyle, budget: { remaining: number }): FountainInlineSegment[] {
  ...
  while (index < text.length) {
    ...
    const closeIndex = marker ? findClosingMarker(text, index + marker.length, marker, budget) : -1;
    if (marker && closeIndex !== -1) {
      flush();
      const inner = text.slice(index + marker.length, closeIndex);
      segments.push(...splitInline(inner, styleWithMarker(style, marker), budget));
      index = closeIndex + marker.length;
      continue;
    }
    literal += text[index];
    // A difficult line must not suppress valid emphasis on following lines.
    if (text[index] === "\n") budget.remaining = lineLookaheadBudget(text, index + 1);
    index += 1;
  }
  flush();
  return segments;
...
function findClosingMarker(text: string, from: number, marker: string, budget: { remaining: number }): number {
  let index = from;
  while (index < text.length) {

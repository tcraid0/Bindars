...
// its own handshake, an in-flight admission swallows repeat requests, and
// every other request is guarded on every platform.
export function decideNativeCloseRequest(state: NativeCloseRequestState): NativeCloseRequestOutcome {
  ...
  if (state.actionAdmissionInFlight) return "prevent-silently";

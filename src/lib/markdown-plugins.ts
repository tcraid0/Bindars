...
/** Normalize only allowed schemes before the sanitizer's case-sensitive check. */
function rehypeLowercaseLinkSchemes() {
  return (tree: Root): void => {
    const visit = (node: Root | RootContent): void => {
      if (node.type === "element" && node.tagName === "a") {
        const href = node.properties.href;
        if (typeof href === "string") {
          node.properties.href = href.replace(/^(?:https?|mailto):/i, (scheme) => scheme.toLowerCase());
        }
      }
...
// 4. rehypeSourcePositions: adds source line/column attributes to blocks
// 5. rehypeLowercaseLinkSchemes: lets the case-sensitive sanitizer accept
//    HTTP/HTTPS/MAILTO links; it must run immediately before the sanitizer
// 6. rehype-sanitize: strips unsafe HTML (preserves math nodes via schema)
// 7. rehype-katex: converts math nodes to KaTeX HTML (runs AFTER sanitize

...
import { decodeUriComponentSafe } from "./paths";
...
export function findFragmentElement(root: HTMLElement, id: string | null): HTMLElement | null {
  if (!id) return null;
  // Footnote IDs retain URI escapes; heading IDs use the decoded spelling.
  // Resolve the original spelling first so labels such as %61 and a stay distinct.
  const decoded = decodeUriComponentSafe(id);
  const exact = findElementById(root, "[id]", id) ?? findElementById(root, "[id]", decoded);
  if (exact) return exact;
  // Also accept authored Unicode or differently cased percent escapes.
  for (const element of root.querySelectorAll<HTMLElement>("[id]")) {
    if (decodeUriComponentSafe(element.id) === decoded) return element;
  }
  return null;
}

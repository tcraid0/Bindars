...
interface EmbedImagesOptions {
  readImageAsBase64?: (path: string, documentPath: string) => Promise<string>;
...
export async function embedImages(
  ...
  if (!documentPath) {
    return { html, embeddedCount: 0, failedCount: assetSources.size };
  }
  ...
  for (const src of assetSources) {
    ...
    try {
      const base64 = await readImageAsBase64(path, documentPath);
...
async function readImageFileAsBase64(path: string, documentPath: string): Promise<string> {
  return invoke<string>("read_image_file_as_base64", { path, documentPath });
}

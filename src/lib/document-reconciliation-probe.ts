...
export async function probeDocumentForReconciliation(
  ...
  switch (outcome.status) {
    case "fulfilled":
      if (toPathIdentityKey(outcome.value.canonicalPath) !== toPathIdentityKey(path)) {
        return reconciliationProbeFailure({
          category: "resource-unavailable",
          message: "The document's path now points to a different location. Your current document remains open. Reopen the file, or use Save As to keep your edits.",
        });
      }
      return {
        status: "available",

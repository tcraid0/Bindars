...
export function useDocumentReconciliation({
  ...
  const runProbe = useCallback((
    ...
    if (captured.userOpenInFlight || captured.guardedActionInFlight || captured.saveInFlight) {
  ...
  const resumeDeferredReconciliation = useCallback(() => {
    ...
    if (current.userOpenInFlight || current.guardedActionInFlight || current.saveInFlight) {

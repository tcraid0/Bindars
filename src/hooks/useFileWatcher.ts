...
export function useFileWatcher({
  ...
  const unavailable = !isEditing && filePath !== null && unavailablePath === filePath;
  const retry = useCallback(() => {
    if (unavailable) setRetryVersion((version) => version + 1);
  }, [unavailable]);
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      try {
        unlistenWatcherUnavailable = await listen<FileWatcherPathEvent>(
          ...
          (event) => {
            ...
            if (!unavailablePath || unavailablePath !== filePath) return;
            healthLost = true;
            setUnavailablePath(unavailablePath);
      ...
      if (!healthLost) setUnavailablePath(null);
  ...
  }, [enqueueWatcherCommand, filePath, isEditing, retryVersion]);
